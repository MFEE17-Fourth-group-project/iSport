        ...
        <div className="relative h-full w-full">
            <div className="relative h-72 2xs:h-80 xs:h-96 sm:h-112 md:h-124 lg:h-136 xl:h-175 w-full overflow-hidden">
                {imageData.map((image, imageIndex) => {
                    ...
                    let position = "h-full w-full object-cover absolute top-0 transition duration-200 ease-in-out transform translate-x-full z-0";

                    if (imageIndex === index) {
                        position = "h-full w-full object-cover absolute top-0 transition duration-200 ease-in-out z-10";
                    }
                    if (imageIndex === index - 1 || (index === 0) && imageIndex === imageData.length - 1) {
                        position = "h-full w-full object-cover absolute top-0 transition duration-200 ease-in-out transform -translate-x-full z-0";
                    }
